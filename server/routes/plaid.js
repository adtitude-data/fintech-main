...
router.post('/create_link_token/:clientId', async function (request, response) {
  const configs = {
    user: {
      // This should correspond to a unique id for the current user.
      client_user_id: 'app-user-id-' + request.params.clientId,
    },
  ...
  const usersAccessTokens = await models.AccountsTokens.findAll({
    where: {
  ...
  /* if(usersAccessTokens.length > 0){
    var tokensAr = [];
    for(var ki = 0 ; ki < usersAccessTokens.length ; ki++) tokensAr.push(usersAccessTokens[ki].access_token);
    if(tokensAr.length > 0) {
      configs.access_token =  tokensAr[0];
...
router.post(
  '/create_link_token_for_payment:clientId',
...
router.post('/set_access_token', async function (request, response, next) {
  ...
  try {
    ...
    var inst_name = instResponse.data.institution.name;
    ...
    const doesAccountExits = await models.AccountsTokens.findOne({
      where: { inst_id: inst_id , client_id : request.body.clientid },
    });
...
router.get('/cronjob/balance/:accountId', async function (request, response, next) {
  ...
  var listOfAccounts = await models.AccountsTokens.findOne({
    attributes: ['client_id', 'inst_name','item_id', 'access_token'],
    where: {
      id: request.params.accountId
    }
  });

  var DBaccountIds = await models.PlaidAccounts.findAll({
    attributes: ['inst_account_id', 'plaid_account_id','status'],
    where: {
      inst_account_id: request.params.accountId
    }
  });
      ...
      if(balanceResponse.data.accounts.length > 0){
        ...
        for(var i = 0 ; i < balanceResponse.data.accounts.length ; i++){
          ...
          if(DBaccountIds.length > 0){ 
            if(__checkIfDeleted(x.account_id , DBaccountIds)){
              canSave = false;
            } 
          }
          ...
          if(canSave){
            dbCreateBalanceOjb.push({
              client_id: client_id ,
              inst_account_id: account_id ,
              plaid_account_id: x.account_id ,
...
router.get('/cronjob/liabilities/:accountId', async function (request, response, next) {
  ...
  console.log('DBaccountIds',DBaccountIds);
...
router.get('/cronjob/investment-holdings/:accountId', async function (request, response, next) {
  ...
  if(listOfAccounts){
    ...
    try {
      ...
      console.log('investmentsHoldingsResponse');
      prettyPrintResponse(investmentsHoldingsResponse);
...
const __checkIfDeleted  = function (plaidLiveAccountId , dbPlaidData){
  var x = dbPlaidData;
  var y = [];
  for(var i = 0 ; i < x.length ; i++){
    if(x[i].status == 2) y.push(x[i].plaid_account_id);
  }
  if(y.includes(plaidLiveAccountId)){
    return true;
  }
  return false;
}
